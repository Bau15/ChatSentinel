...
public final class BungeeFilterFileLoader {
    ...
    private FilterExpressionFile loadFile(FilterKind kind, File file, String moduleId, String relativePath) {
        try {
            ...
            FilterSource source = new FilterSource(kind, moduleId, relativePath, moduleId);
            return new FilterExpressionFile(source, Collections.<String>emptyList());
        }
    }
    ...
    private void copyDefaultFolderFiles(String folderName, File targetFolder) {
        try {
            Enumeration<URL> resources = BungeeFilterFileLoader.class.getClassLoader().getResources(folderName);
    ...
    private void copyResource(String resourcePath, File targetFile) throws IOException {
        ...
        InputStream inputStream = BungeeFilterFileLoader.class.getClassLoader().getResourceAsStream(resourcePath);
        if (inputStream == null) {
            return;
        }
        try {
